...
export default function Home() {
  return (
    <>
      ...
      <script src="https://unpkg.com/downloadjs@1.4.7/download.min.js" />
      <script dangerouslySetInnerHTML={{__html:`
// ── STATE ──
const files=[];let activeId=null,splitMethod='pages',proofLevel='normal';
...
async function handleFiles(list){
  for(const f of list){
    if(!f.name.endsWith('.pdf')){toast(f.name+': PDF만 지원합니다','warning');continue;}
    await loadPDF(f);
  }
}
async function loadPDF(f){
  ...
  let pageCount=0;
  try{const doc=await PDFLib.PDFDocument.load(ab,{ignoreEncryption:true});pageCount=doc.getPageCount();}
  catch(e){toast(f.name+': 읽기 실패','error');return;}
  const entry={id:uid(),name:f.name,size:f.size,pageCount,arrayBuffer:ab,chunks:[],proofResults:[]};
  files.push(entry);
  renderFileList();selectFile(entry.id);updateTopbarMeta();
...
function renderFileList(){
  ...
  if(!files.length){list.innerHTML='<div style="padding:20px;text-align:center;color:var(--text3);font-size:12px;">업로드된 파일이 없습니다</div>';return;}
  list.innerHTML=files.map(f=>{
    ...
    return '<div class="file-item '+(f.id===activeId?'active':'')+'" onclick="selectFile(\''+f.id+'\')">'
      ...
      +'<div class="file-meta">'+f.pageCount+'p · '+fmtSize(f.size)+(done?' · 교정완료':'')+'</div></div>'
      +'<div class="file-status '+(done?'status-done':'status-new')+'"></div></div>';
  }).join('');
}
...
function selectFile(id){
  ...
  renderSplitTab(f);renderProofreadTab(f);
...
function renderSplitTab(f){
  ...
  el.innerHTML=
    ...
    +'<div class="split-method-grid">'
      +['pages','size','range','all'].map((m,i)=>{
        const icons=['📃','⚖️','🎯','📑'];
        const names=['페이지 수','파일 크기','범위 지정','1페이지씩'];
        const descs=['N페이지씩 균등 분할','MB 단위로 분할','직접 페이지 범위 입력','모든 페이지 개별 분리'];
        return '<div class="method-card '+(splitMethod===m?'selected':'')+'" onclick="setSplitMethod(\''+m+'\',\''+f.id+'\')">'
          +'<span class="method-icon">'+icons[i]+'</span>'
          +'<div class="method-name">'+names[i]+'</div>'
          +'<div class="method-desc">'+descs[i]+'</div></div>';
      }).join('')
    +'</div>'
...
function setSplitMethod(m,fid){splitMethod=m;const f=files.find(x=>x.id===fid);if(f)renderSplitTab(f);}
function renderSplitOptions(f){
  const el=document.getElementById('splitOptions');if(!el)return;
  if(splitMethod==='pages'){
    const rec=Math.max(1,Math.min(30,Math.ceil(f.pageCount/5)));
    el.innerHTML='<div class="form-row"><span class="form-label">분할 단위 (페이지)</span><div class="range-wrap"><input type="range" id="splitPages" min="1" max="'+f.pageCount+'" value="'+rec+'" oninput="document.getElementById(\'splitPagesVal\').textContent=this.value;updatePreview(\''+f.id+'\')"><span class="range-value" id="splitPagesVal">'+rec+'</span></div></div>';
    setTimeout(()=>updatePreview(f.id),0);
  }else if(splitMethod==='size'){
    const rec=Math.max(1,Math.round(f.size/f.pageCount/(1024*1024)*10));
    el.innerHTML='<div class="form-row"><span class="form-label">최대 크기 (MB)</span><input type="number" class="form-input" id="splitSize" min="0.5" max="500" step="0.5" value="'+(rec||5)+'" oninput="updatePreview(\''+f.id+'\')"></div><div class="form-hint">파일 크기는 추정값입니다</div>';
    setTimeout(()=>updatePreview(f.id),0);
  }else if(splitMethod==='range'){
    el.innerHTML='<div class="form-row"><span class="form-label">페이지 범위</span><input type="text" class="form-input" id="splitRange" placeholder="예: 1-10, 11-25, 26-'+f.pageCount+'" oninput="updatePreview(\''+f.id+'\')"></div><div class="form-hint">쉼표로 구분, 총 '+f.pageCount+'페이지</div>';
  }else{
...
function computeChunks(f){
  ...
  if(splitMethod==='pages'){
    ...
    for(let s=1;s<=total;s+=n){const e=Math.min(s+n-1,total);chunks.push({label:'파트'+(chunks.length+1),pages:s+'–'+e,count:e-s+1,size:Math.round(f.size*(e-s+1)/total),start:s,end:e});}
  }else if(splitMethod==='size'){
    const mb=parseFloat(document.getElementById('splitSize')?.value||5);
    const ppc=Math.max(1,Math.floor(mb*1024*1024*total/f.size));
    for(let s=1;s<=total;s+=ppc){const e=Math.min(s+ppc-1,total);chunks.push({label:'파트'+(chunks.length+1),pages:s+'–'+e,count:e-s+1,size:Math.round(f.size*(e-s+1)/total),start:s,end:e});}
  }else if(splitMethod==='range'){
    const parts=(document.getElementById('splitRange')?.value||'').split(',').map(r=>r.trim()).filter(Boolean);
    for(const p of parts){const m=p.match(/^(\d+)-(\d+)$/);if(!m)continue;const s=+m[1],e=+m[2];if(s<1||e>total||s>e)continue;chunks.push({label:'파트'+(chunks.length+1),pages:s+'–'+e,count:e-s+1,size:Math.round(f.size*(e-s+1)/total),start:s,end:e});}
  }else{
    for(let i=1;i<=total;i++)chunks.push({label:'페이지'+i,pages:''+i,count:1,size:Math.round(f.size/total),start:i,end:i});
  }
  return chunks;
}
function updatePreview(fid){
  ...
  const preview=document.getElementById('splitPreview');if(!preview)return;
  if(!chunks.length){preview.innerHTML='<div style="color:var(--text3);font-size:12px;padding:8px 0;">유효한 분할 설정을 입력하세요</div>';return;}
  const show=(splitMethod==='all'&&chunks.length>8)?chunks.slice(0,8):chunks;
  const more=chunks.length-show.length;
  preview.innerHTML='<div class="preview-box"><table class="preview-table"><thead><tr><th>#</th><th>이름</th><th>범위</th><th>페이지</th><th>크기</th></tr></thead><tbody>'
...
function renderProofreadTab(f){
  ...
  el.innerHTML='<div class="section-title">교정 강도</div>'
    +'<div class="level-grid">'
      +[['light','🔍','가볍게','맞춤법·띄어쓰기만'],['normal','✏️','보통','어색한 표현 포함'],['deep','🔬','꼼꼼히','논리 흐름까지']].map(([lv,ic,nm,dc])=>
        '<div class="level-card '+(proofLevel===lv?'selected':'')+'" onclick="setProofLevel(\''+lv+'\')">'
        +'<span class="level-icon">'+ic+'</span><div class="level-name">'+nm+'</div><div class="level-desc">'+dc+'</div></div>'
      ).join('')
    +'</div>'
    +'<div class="divider"></div>'
    +'<div class="section-title">교정 대상</div>'
    +'<div style="font-size:13px;color:var(--text2);margin-bottom:12px;">분할된 청크 단위로 순차 교정합니다. 분할하지 않은 경우 전체 텍스트를 한 번에 교정합니다.</div>'
...
function setProofLevel(lv){proofLevel=lv;const f=files.find(x=>x.id===activeId);if(f)renderProofreadTab(f);}

// ── TAB: 결과 ──
function renderResultTab(f){
  const el=document.getElementById('content-result');
  if(!f.proofResults.length){el.innerHTML='<div style="color:var(--text3);font-size:13px;padding:20px 0;">교정을 실행하면 결과가 여기에 표시됩니다</div>';return;}
  const allChanges=f.proofResults.flatMap(r=>r.changes||[]);
  const summaries=f.proofResults.map(r=>r.summary).filter(Boolean);
  let html='';
  if(summaries.length)html+='<div class="summary-box"><strong>📝 교정 요약</strong><br>'+summaries.map(escHtml).join(' / ')+'</div>';
  if(allChanges.length){
    html+='<div class="section-title" style="margin-bottom:10px;">변경 사항 ('+allChanges.length+'건)</div>'
      ...
      +allChanges.map(c=>{
        ...
        return '<tr><td><mark class="del">'+escHtml(c.before||'')+'</mark></td>'
          +'<td><mark class="ins">'+escHtml(c.after||'')+'</mark></td>'
          +'<td style="color:var(--text2)">'+escHtml(c.reason||'')+'</td>'
          +'<td><span class="tag '+tc+'">'+escHtml(c.type||'기타')+'</span></td></tr>';
      }).join('')
      +'</tbody></table></div>';
  }
...
async function doSplit(fid){
  ...
  const ps=document.getElementById('progressSection');if(ps)ps.style.display='block';
  const btn=document.getElementById('btnSplit');if(btn)btn.disabled=true;
  try{
    ...
    f.chunks=results;renderFileList();selectFile(fid);
    if(pt)pt.textContent='✅ 분할 완료 — '+results.length+'개 파일';
    toast('분할 완료: '+results.length+'개 파일','success');
  }catch(e){toast('분할 오류: '+e.message,'error');}
  const btn2=document.getElementById('btnSplit');if(btn2)btn2.disabled=false;
}
async function doDownloadAll(fid){
  const f=files.find(x=>x.id===fid);if(!f||!f.chunks.length)return;
...
async function doProofread(fid){
  const f=files.find(x=>x.id===fid);if(!f)return;
  const btn=document.getElementById('btnProof');if(btn)btn.disabled=true;
  ...
  try{
    ...
    renderFileList();renderResultTab(f);selectFile(fid);switchTab('result');
    toast('교정 완료!','success');
  }catch(e){
    toast('교정 오류: '+e.message,'error');
    if(pt)pt.textContent='❌ 오류: '+e.message;
  }
  const btn2=document.getElementById('btnProof');if(btn2)btn2.disabled=false;
}

async function extractTextFromPDF(buffer){
  try{
    const uint8=new Uint8Array(buffer instanceof ArrayBuffer?buffer:await new Response(buffer).arrayBuffer());
    ...
    for(const block of btEt){
      ...
      for(const tf of tfM){const inner=tf.match(/\\[([^\\]]*)\\]/)[1];const sp=inner.match(/\\(([^)]*)\\)/g)||[];for(const s of sp)text+=s.slice(1,-1)+' ';}
    }
    return text.trim()||'(텍스트 레이어 없음 — 스캔 PDF일 수 있습니다)';
  }catch(e){return '(텍스트 추출 실패: '+e.message+')';}
}

function downloadResult(fid){
  const f=files.find(x=>x.id===fid);if(!f||!f.proofResults.length)return;
...
function removeFile(fid){
  const idx=files.findIndex(x=>x.id===fid);if(idx===-1)return;
  files.splice(idx,1);activeId=null;renderFileList();
  document.getElementById('detailPanel').classList.remove('visible');
  document.getElementById('emptyState').style.display='';
  updateTopbarMeta();toast('파일이 제거되었습니다','info');
}

function updateTopbarMeta(){
  const meta=document.getElementById('topbarMeta');
  if(!files.length){meta.textContent='PDF 파일을 업로드하여 시작하세요';return;}
  meta.textContent=files.length+'개 파일 · '+files.reduce((a,f)=>a+f.pageCount,0)+'페이지 · '+fmtSize(files.reduce((a,f)=>a+f.size,0));
}

// ── INIT ──
window.switchTab=switchTab;window.setSplitMethod=setSplitMethod;window.setProofLevel=setProofLevel;
window.selectFile=selectFile;window.doSplit=doSplit;window.doDownloadAll=doDownloadAll;
window.doProofread=doProofread;window.downloadResult=downloadResult;window.removeFile=removeFile;window.updatePreview=updatePreview;
